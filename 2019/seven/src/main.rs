...
fn run(state: &mut Vec<i64>, offset: &mut usize, input: Vec<i64>) -> ICResult {
    // println!("running {:?} with {:?}", state, input);
    ...
    loop {
        ...
        match op.code {
            ...
            4 => {
                output.output = op.param(state, *offset, 1);
...
impl Amp {
    fn exec(&mut self, input: i64) -> ICResult {
        let r = match self.running {
            true => run(&mut self.state, &mut self.offset, vec![input]),
            false => {
                self.running = true;
                run(&mut self.state, &mut self.offset, vec![self.phase, input])
            }
        };
        self.done = r.done;
        r
...
fn run_phases(state: &mut Vec<i64>, phases: &[i64]) -> i64 {
...
fn main() {
    ...
    let mut signal = 0;
    heap_recursive(&mut vec![9, 8, 7, 6, 5], |p| {
        // println!("trying permuation {:?}", p);
        let s = run_phases(&mut program.clone(), p);
        if s > signal {
            // println!("s({}) > signal({})", s, signal);
            signal = s;
        }
    });
    println!("largest signal was: {}", signal);
}
...
mod test {
    ...
    fn test_big() {
        ...
        assert_eq!(
            run_phases(&mut program.clone(), &vec![9, 8, 7, 6, 5]),
            139629729
        );
    }
    ...
    fn test_long() {
        ...
        assert_eq!(
            run_phases(&mut program.clone(), &vec![9, 7, 8, 5, 6]),
            18216
        );
    }
    ...
    fn test_43210() {
        ...
        assert_eq!(
            run_phases(&mut program.clone(), &vec![4, 3, 2, 1, 0]),
            43210
        );
    }
    ...
    fn test_cat() {
        ...
        let mut offset = 0;
        let output = run(&mut program, &mut offset, vec![1]).output;
    ...
    fn test_multiply() {
        ...
        let mut offset = 0;
        let _output = run(&mut program, &mut offset, vec![1]).output;
    }
    ...
    fn test_equal_to_eight() {
        ...
        assert_eq!(run(&mut program.clone(), &mut 0, vec![8]).output, 1);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![7]).output, 0);
    }

    #[test]
    fn test_less_than_eight() {
        let program = vec![3, 9, 7, 9, 10, 9, 4, 9, 99, -1, 8];
        assert_eq!(run(&mut program.clone(), &mut 0, vec![8]).output, 0);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![9]).output, 0);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![3]).output, 1);
    }
    ...
    fn test_equal_to_eight_im() {
        ...
        assert_eq!(run(&mut program.clone(), &mut 0, vec![8]).output, 1);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![5]).output, 0);
    }

    #[test]
    fn test_less_than_eight_im() {
        let program = vec![3, 3, 1107, -1, 8, 3, 4, 3, 99];
        assert_eq!(run(&mut program.clone(), &mut 0, vec![2]).output, 1);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![9]).output, 0);
    }
    ...
    fn test_jump_pos() {
        ...
        assert_eq!(run(&mut program.clone(), &mut 0, vec![0]).output, 0);
    }

    #[test]
    fn test_jump_im() {
        let program = vec![3, 3, 1105, -1, 9, 1101, 0, 0, 12, 4, 12, 99, 1];
        assert_eq!(run(&mut program.clone(), &mut 0, vec![1]).output, 1);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![2]).output, 1);
    }
    ...
    fn test_lt_8_cmp() {
        ...
        assert_eq!(run(&mut program.clone(), &mut 0, vec![5]).output, 999);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![8]).output, 1000);
        assert_eq!(run(&mut program.clone(), &mut 0, vec![15]).output, 1001);
    }
}
