...
fn get_orbits<'a>(orbits: &'a HashMap<&str, &str>, subset: &mut Vec<&'a str>, initial_key: &str) -> usize {
...
fn main() -> Result<(), std::io::Error> {
    ...
    let orbits: HashMap<&str, &str> = buffer.lines().map(|line| {
        let pair: Vec<&str> = line.split(")").collect();
        (pair[1], pair[0])
    }).collect();

    let total: usize = orbits.keys().map(|k| {
        let mut subset = vec![];
    ...
    let mut steps: Vec<_> = intersections.iter().map(|point| {
